...
function App() {
  ...
  //   ★알림센터(트레이) 일괄 정리는 하지 않는다(2026-07-21) — 푸시가 여러 개 쌓였을 때 하나만 탭해도
  //    앱이 열리며 나머지가 전부 지워져 "뭘 안 봤는지" 알 수 없게 되던 원인. 탭한 알림은 OS가 알아서 지우고,
  //    안 본 알림은 트레이에 남겨 사용자가 확인처로 쓴다. DM·초대 등 인앱 알림함에 문서가 안 남는 종류도 있어
  //    트레이를 지우면 복구할 방법이 없었음.
  useEffect(() => {
    const clearBadge = () => {
