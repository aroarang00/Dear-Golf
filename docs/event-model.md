...
## 14. 조 편성 UI (MVP 핵심 화면)
**진입**: 라운지 [확정] / 전파 5인+·"여러 조로 나누기" → 시트(모달).

**상단**
- 타이틀 `조 편성` + 요약 `○○CC · 6.28(토) · 12명 / 3조`
- 조 수: 기본 `ceil(N/4)`, `[+조] [−조]`

**미배정 풀** (상단)
- `미배정 (n)` + 이름 칩. 칩 탭 = 선택(테두리 강조).

**조 카드** (조1·조2·…)
- 헤더 `1조 · 3/4` (4 초과 빨강 경고)
- 멤버 칩(이름 + `✕`=미배정 복귀)
- `+ 멤버`(미배정 체크 다중선택 추가)
- **티오프** `[HH:MM]` + `미정` 토글
- **세부코스** `[입력]`(선택)
- 예약자 booker: 접힘/고급, 기본 단일

**배정 인터랙션 — 드래그 라이브러리 없이**
- 기본: 미배정 칩 **탭(선택) → 대상 조 헤더 탭 → 이동**(의존성 0).
- 보조: 각 조 `+ 멤버` 체크 추가 / 멤버 `✕` 빼기.

**보조 도구 (둘 다 유지, 자동 강제 아님)**
- `순서대로 채우기` — 미배정을 4명씩 자동 배치(편의 버튼).
- `티오프 일괄` — **첫 조 시각 + 간격(분)** 입력 → 조별 자동 채움(예: 07:30/간격8 → 07:30·07:38·07:46), 이후 개별 수정. **간격은 입력값(구장별 상이, 하드코딩 X).**

**검증·푸터**
- 조 ≤4 강제, **3인 조 허용**, 동일인 중복 배정 불가.
- 미정 티오프 허용. 미배정 남으면 경고(확정 차단 또는 확인).
- `[취소] [확정 → 일정 생성]` (확정 시 생성 CF `createGroupTeams` 호출).

**생성 후**: 각자 자기 조 카드, 주최자에 토스트 `3조 12명 일정 생성 완료`.
