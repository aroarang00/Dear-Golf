...
export function normalizeCourseName(name) {
  ...
  s = s.replace(/컨트리클럽|컨트리|골프앤리조트|골프&리조트|골프링크스|골프리조트|골프클럽|골프장|골프|리조트/g, '');
  s = s.replace(/c\.?c\.?(?![a-z])|g\.?c\.?(?![a-z])/g, '');
  // 코스 구분어 제거 — 끝에 오는 '(수식어)코스' (다중 코스 골프장: 라비에벨 올드/뉴 등)
  s = s.replace(/(올드|뉴|이스트|웨스트|사우스|노스|동|서|남|북|챔피언|퍼블릭|레이크|마운틴|밸리|힐|레이디스|old|new|east|west|south|north)?(코스|course)$/g, '');
  s = s.replace(/[·.,&\-_'"()]/g, '');
  return s;
}
