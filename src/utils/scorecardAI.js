...
// 스코어카드 사진 선택 — 갤러리는 다중(태블릿 전/후반 2장 또는 전체 카드 팀별 최대 4장), 카메라는 1장.
//   반환: { uris:[...] } | { denied:true } | null(취소).
export async function pickScorecardImages(source = 'gallery', max = 4) {
  if (source === 'camera') {
...
// 스코어카드 AI 추출 — 사진 1~4장 → Gemini 비전(extractScorecard CF).
//   ① 태블릿 전/후반 조각(9홀씩)이면 병합 ② 전체 카드(18홀·여러 명)면 카드마다 전원 추출(단체=팀별 최대 4장→최대 16명).
...
// 사진 1~4장(uri) → { rows:[{ label, holes:number[18], total }], holePars:number[18]|null } | { error }
//   rows = 플레이어(행)별(여러 카드/여러 팀이면 전원). 여러 명이면 검토 모달이 '본인 행 선택' UI를 띄움.
export async function extractScorecardAI(uris) {
  try {
    const list = (Array.isArray(uris) ? uris : [uris]).filter(Boolean).slice(0, 4);
