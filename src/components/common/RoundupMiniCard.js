...
export function RoundupMiniCard({ roundupId, post: preloaded = null, onPress }) {
  ...
  const [state, setState] = useState(preloaded ? (preloaded.cancelledByHost ? 'gone' : 'ok') : 'loading'); // loading | ok | gone

  useEffect(() => {
    // 상위가 모집 doc을 이미 들고 있으면(상단 핀) 그대로 사용 — 중복 조회 회피.
    if (preloaded) { setPost(preloaded); setState(preloaded.cancelledByHost ? 'gone' : 'ok'); return; }
