...
const OVERLAYS = {
  ...
  // ★2026-07-09 재완화 — 하단 0.82가 페어웨이(밝은 초록)를 청회색으로 죽여, 사진이 통째로
  //   '구름 낀 하늘 한 덩어리'로 보였다(테스터 2회 신고, 실기기 스크린샷 픽셀 측정으로 확인).
  //   사진은 정상 렌더 중이었고 범인은 이 오버레이였다. 하단 0.82→0.58, 가운데 0.48→0.38.
  //   상단(0.86→0.80)은 'Dear Golf'·인사말 가독을 위해 여전히 짙게 유지.
  rain:   ['rgba(16,24,34,0.80)', 'rgba(16,24,34,0.38)', 'rgba(16,24,34,0.40)', 'rgba(16,24,34,0.58)'],
};
