  ...
  if (onPress) {
    return (
      <TouchableOpacity onPress={onPress} activeOpacity={0.6}
        ...
        <Text style={{ fontSize: fs(size) }}>{g.emoji}</Text>
      </TouchableOpacity>
    );
  }
  return <Text style={{ fontSize: fs(size) }}>{g.emoji}</Text>;
}
      ...
      <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'center', paddingHorizontal: 32 }}
        ...
        <View style={{ backgroundColor: C.bgPrimary, borderRadius: 16, overflow: 'hidden' }}>
          <View style={{ backgroundColor: C.navy, paddingVertical: 16, paddingHorizontal: 18 }}>
            ...
            <Text style={{ fontFamily: F.sys, fontSize: fs(11), color: 'rgba(250,246,236,0.7)', marginTop: 4, lineHeight: 16 }}>
              동반자 평가에 따라 등급이 변해요{'\n'}디어골프 처음 이용하는 분은 보통부터 시작해요
            </Text>
          </View>
            ...
            {[...MANNER_GRADES].reverse().map((g) => {
              ...
              return (
                <View key={g.key}
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontFamily: F.sysB, fontSize: fs(13), color: g.color }}>{g.label}</Text>
