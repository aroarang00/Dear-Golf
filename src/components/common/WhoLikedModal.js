      ...
      <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'center', paddingHorizontal: 40 }}
        ...
        <View style={{ backgroundColor: C.bgPrimary, borderRadius: 16, overflow: 'hidden' }}>
          <View style={{ paddingVertical: 14, borderBottomWidth: 0.5, borderBottomColor: C.hairline }}>
            <Text style={{ fontFamily: F.sys, fontSize: 14, color: C.charcoal, fontWeight: '700', textAlign: 'center' }}>
              👍 좋아요 {names.length}
            </Text>
          </View>
          {names.length === 0 ? (
            <Text style={{ fontFamily: F.sys, fontSize: 12, color: C.warmGrayLight, textAlign: 'center', paddingVertical: 28 }}>
              아직 좋아요가 없어요
            </Text>
          ) : (
              ...
              {names.map((nm, i) => (
                <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 18, paddingVertical: 9 }}>
                  <View style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: AV[i % AV.length],
                    ...
                    <Text style={{ fontFamily: F.sys, fontSize: 15, color: C.charcoal, fontWeight: '700' }}>{nm.charAt(0)}</Text>
                  </View>
                  <Text style={{ fontFamily: F.sys, fontSize: 14, color: C.charcoal, fontWeight: nm === '나' ? '700' : '500' }}>
                    {nm}
                  </Text>
                </View>
              ))}
