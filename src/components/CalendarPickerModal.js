              ...
              {rows.map(c => {
                ...
                return (
                  <TouchableOpacity key={c.id} onPress={() => pick(c.id)} activeOpacity={0.7}
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontFamily: F.sys, fontSize: 14, color: C.charcoal, fontWeight: '600' }}>{c.title}</Text>
                      <Text style={{ fontFamily: F.sys, fontSize: 11, color: C.warmGrayLight, marginTop: 2 }}>{c.account}</Text>
                    </View>
                    {on && <Text style={{ fontSize: 16, color: C.burgundy, fontWeight: '700' }}>✓</Text>}
                  </TouchableOpacity>
                );
              })}
              {cals.length === 0 && (
                <Text style={{ fontFamily: F.sys, fontSize: 12, color: C.warmGray, paddingVertical: 18, textAlign: 'center', lineHeight: 18 }}>
                  연동 가능한 캘린더가 없어요.{'\n'}휴대폰 설정에서 캘린더 권한을 확인해주세요.
                </Text>
              )}
            </ScrollView>
          )}
          <Text style={{ fontFamily: F.sys, fontSize: 11, color: C.warmGrayLight, marginTop: 14, textAlign: 'center', lineHeight: 16 }}>
