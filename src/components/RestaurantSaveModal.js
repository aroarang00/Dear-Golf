      ...
      <KeyboardAvoidingView
        ...
        <TouchableOpacity activeOpacity={1} onPress={onClose}
          ...
          <TouchableOpacity activeOpacity={1} onPress={() => {}}
            ...
            <Text style={{ fontFamily: 'Georgia', fontStyle: 'italic', fontSize: 19, color: C.charcoal }}>
              {isEdit ? '메모 수정' : '맛집 저장'}
            </Text>
            <Text style={{ fontFamily: F.sys, fontSize: 11, color: C.warmGray, marginTop: 3, marginBottom: 16 }}>
              {courseName ? `${courseName} · ` : ''}{isEdit ? '저장한 맛집' : (fromKakao ? '카카오 검색 결과' : '직접 추가')}
            </Text>

            <Text style={{ fontFamily: F.sys, fontSize: 11, color: C.warmGray, marginBottom: 6 }}>맛집 이름</Text>
            <TextInput
              value={name}
              onChangeText={setName}
              placeholder="맛집 이름"
              placeholderTextColor={C.warmGrayLight}
              style={[inputBase, { fontSize: 14, marginBottom: fromKakao && seed?.loc ? 6 : 14 }]}
            />
            {fromKakao && !!seed?.loc && (
              <Text style={{ fontFamily: F.sys, fontSize: 11, color: C.warmGrayLight, marginBottom: 14 }}>
                📍 {seed.loc}
              </Text>
            )}

            <Text style={{ fontFamily: F.sys, fontSize: 11, color: C.warmGray, marginBottom: 6 }}>메모 (선택)</Text>
            <TextInput
              value={memo}
              ...
              placeholderTextColor={C.warmGrayLight}
              multiline
              style={[inputBase, { fontSize: 13, minHeight: 64, textAlignVertical: 'top', marginBottom: 18 }]}
            />
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              ...
              <TouchableOpacity onPress={handleSave} disabled={!canSave}
                ...
                <Text style={{ fontFamily: F.sys, fontSize: 13, fontWeight: '600', color: canSave ? C.butter : C.warmGrayLight }}>{isEdit ? '수정' : '저장'}</Text>
              </TouchableOpacity>
            </View>
          </TouchableOpacity>
        </TouchableOpacity>
