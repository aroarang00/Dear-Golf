...
//   ★갤러리 탭(사진 낱장 3열)과 겹치지 않는 이유 = 여기는 사진이 아니라 글이 단위다. 사진 없는 글도 한 칸을 갖는다.
//   탭=댓글 화면(원글 전체가 위에 뜨므로 사실상 상세). 좋아요·더보기(⋯)는 큰 카드에만 있던 동작이라 여기로 옮겨왔다.
const PostGridCard = React.memo(function PostGridCard({ p, cell, onOpen, onToggleLike, onAction, onOpenProfile, onOpenLikers, onOpenRoundup }) {
  const cover = p.media?.length ? p.media[0] : null;
  const more = (p.media?.length || 0) - 1;
  // 라운지 모집을 크루에 공유한 글 — 사진이 없어 글자만 남던 자리에 모집 타일(구장·날짜·상태·인원)을 채운다.
  ...
  return (
    <TouchableOpacity activeOpacity={0.9} style={{ width: cell, marginBottom: 16 }}
      ...
      <View>
        ...
        {more > 0 && (
          <View style={{ position: 'absolute', top: 8, right: 8, backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 9, paddingHorizontal: 7, paddingVertical: 2 }}>
            <Text style={{ fontFamily: F.sysB, fontSize: fs(10.5), color: '#fff' }}>+{more}</Text>
          </View>
        )}
        {p.isNew && (
          <View style={{ position: 'absolute', top: 8, left: 8, backgroundColor: NEWMARK, borderRadius: 7, paddingHorizontal: 6, paddingVertical: 1.5 }}>
            <Text style={{ fontFamily: F.sysB, fontSize: fs(9), color: '#fff', letterSpacing: 0.4 }}>NEW</Text>
          </View>
        )}
      </View>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6 }}>
        <TouchableOpacity onPress={() => onToggleLike(p)} hitSlop={{ top: 10, bottom: 10, left: 8, right: 8 }}
          ...
          {/* 숫자 탭 = 누가 눌렀나 (하트 탭은 토글) — 1열 카드와 같은 규칙 */}
          {p.likeCount > 0 && (
            <Text onPress={() => onOpenLikers(p)} suppressHighlighting
              style={{ fontFamily: F.sysM, fontSize: fs(11.5), color: SUB, marginLeft: 3 }}>{p.likeCount}</Text>
          )}
        </TouchableOpacity>
        {p.comments > 0 && (
          <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 11 }}>
            {p.hasNewComment && <View style={{ width: 5, height: 5, borderRadius: 2.5, backgroundColor: NEWMARK, marginRight: 4 }} />}
            <Text style={{ fontFamily: F.sysM, fontSize: fs(11.5), color: p.hasNewComment ? INK : SUB }}>댓글 {p.comments}</Text>
          </View>
        )}
...
export function CrewAlbumScreen({ crew, onClose, onOpenDM, onOpenRoundup, seenAt = 0 }) {
  ...
  const pinnedRoundupIds = useMemo(() => {
    const ids = [];
    // 핀 = '크루에서 만든 모집'만(roundupShare 없음). 공유(roundupShare=true)는 핀 아니라 피드 광고 글로.
    (postDocs || []).forEach((p) => { if (p.roundupId && !p.roundupShare && !ids.includes(p.roundupId)) ids.push(p.roundupId); });
    return ids;
  }, [postDocs]);
  // roundupId→주최자 uid — 비친구라 모집을 못 읽을 때 '주최자와 친구 맺기' 안내 타겟(공유/생성 시 게시글에 저장됨).
  const roundupHostById = useMemo(() => {
    const m = {};
  ...
  // ── 동작 ── (카드(PostGridCard)에 넘기는 콜백은 useCallback으로 안정화 — memo 카드가 UI상태 변화에 헛 리렌더되지 않게)
  ...
  // 게시글 탭 — 2열 카드 그리드.
  //   ★퍼센트 폭(48% 등)과 고정 gap을 섞으면 안 된다. 안드에서 반올림에 밀려 두 번째 칸이 다음 줄로 튕긴다.
  //    남는 폭을 floor로 직접 나눈다(너나픽에서 같은 사고를 겪고 정한 방식). [[sdk57-layout-gotchas]]
  const F_PAD = 14, F_GAP = 10, F_COLS = 2;
  const fcell = Math.floor((winW - F_PAD * 2 - F_GAP * (F_COLS - 1)) / F_COLS);
  ...
  // 게시글 카드 — 2열 그리드. 탭하면 그 글의 댓글 화면(원글 전체가 위에 뜬다).
  const renderFeedItem = ({ item: p }) => (
    <PostGridCard p={p} cell={fcell} onOpen={openComment} onToggleLike={toggleLike} onAction={openPostAction}
      onOpenProfile={openProfile} onOpenLikers={openLikers} onOpenRoundup={onOpenRoundup} />
  );
  ...
  return (
    ...
    <SafeAreaView edges={['top', 'left', 'right']} style={{ flex: 1, backgroundColor: BG }}>
      ...
      <FlatList
        ...
        numColumns={tab === 'feed' ? F_COLS : COLS}
        keyExtractor={(item) => (tab === 'feed' ? item.id : item.key)}
        renderItem={tab === 'feed' ? renderFeedItem : renderTile}
        columnWrapperStyle={tab === 'feed'
          ? { paddingHorizontal: F_PAD, gap: F_GAP }
        ...
        initialNumToRender={tab === 'feed' ? 8 : 12}
        maxToRenderPerBatch={tab === 'feed' ? 8 : 12}
        windowSize={7}
        // 카드 높이가 가변이라 미렌더 인덱스로 점프 시 실패할 수 있음 — 대략 위치로 먼저 이동 후 재시도(배너 '보기'용).
        //   2열이 되며 한 칸 높이가 사진(fcell)+글·지표 정도로 줄어 폴백 추정치도 그에 맞춘다.
        onScrollToIndexFailed={(info) => {
          const guess = tab === 'feed' ? (fcell + 76) / F_COLS : 320;
          scrollRef.current?.scrollToOffset({ offset: (info.averageItemLength || guess) * info.index, animated: true });
